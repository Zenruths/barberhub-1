    ...
    <div class="p-6 space-y-4">
      ...
      <div v-else-if="googleConnected === false" class="space-y-3">
        <p class="text-sm text-gray-700 dark:text-gray-300">
          Para criar agendamentos, é obrigatório conectar sua conta do Google Calendar.
        </p>
        <button
          type="button"
      ...
      <div v-else class="space-y-2">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          Google conectado. (A UI de criação pode ser adicionada aqui.)
        </p>
      </div>
    </div>
  </AdminLayout>
...
defineOptions({ name: 'AgendamentosNovoAgendamento' })
