          ...
          <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
            <div>
              ...
              <div>
                ...
                <div class="space-y-4">
                  <Alert
                    v-if="showSessionEnded"
                    variant="info"
                    title="Sessão finalizada"
                    message="Sua sessão foi encerrada. Faça login novamente para continuar."
                    :closable="true"
                    @close="handleCloseSessionAlert"
                  />
                  <!-- Removido Alert de sucesso local; sucesso será tratado no dashboard -->
                  <form @submit.prevent="handleSubmit">
                    <div class="space-y-5">
                      ...
                      <p v-if="errorMessage" class="mt-2 text-sm text-error-500">{{ errorMessage }}</p>
                    </div>
                  </form>
                </div>
                <div class="mt-5">
          ...
          <div class="flex items-center justify-center z-1">
            ...
            <div class="flex flex-col items-center max-w-xs">
              ...
              <p class="text-center text-gray-400 dark:text-white/60">
                A gestão pode ser fácil, deixa a gente te mostrar o caminho!
              </p>
            </div>
          </div>
        </div>
...
// controle local para permitir fechar manualmente sem depender apenas da query
const showSessionEnded = ref(false)

const sessionEnded = computed(() => route.query.session === 'ended')

onMounted(() => {
  const fromQuery = sessionEnded.value
  const fromStorage = sessionStorage.getItem('session_ended') === '1'

  if (fromQuery || fromStorage) {
    showSessionEnded.value = true

    if (fromQuery) {
      router.replace({ path: route.path, query: { ...route.query, session: undefined } })
    }
  }

  // Se a navegação atual é um reload, limpamos a flag e o alerta.
...
// Reagir a mudanças de rota (ex: logout -> push('/signin')) no mesmo ciclo SPA
watch(
  () => route.fullPath,
  () => {
    const fromQuery = sessionEnded.value
...
const handleSubmit = async (event: Event) => {
  ...
  try {
    ...
    // Ao logar com sucesso, garantimos que a bandeira de sessão encerrada seja removida
    sessionStorage.removeItem('session_ended')

    // Redirecionar para o dashboard indicando sucesso via query
